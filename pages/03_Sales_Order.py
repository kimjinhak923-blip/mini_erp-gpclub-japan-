...
selected_partner_name = col_h2.selectbox(t("partner_select"), list(partner_dict.keys()))
selected_partner = partner_dict[selected_partner_name]

wh_dict = {w["name"]: w["id"] for w in warehouses}
selected_wh_name = col_h3.selectbox(t("wh_select"), list(wh_dict.keys()))
...
address = col_d3.text_input(t("deliv_addr"), value=selected_partner.get("address", ""))
phone = col_d4.text_input(t("deliv_phone"), value=selected_partner.get("phone", ""))
...
# 거래처에 등록된 제품 + 해당 창고 재고 > 0 인 제품만 필터링
...
if not item_options:
    st.error("⚠️ 해당 거래처 지정 제품 중 선택한 창고에 보유 중인 재고(>0)가 없습니다.")
else:
    for idx in range(st.session_state["order_rows"]):
        c_p, c_pr, c_q, c_bx, c_tot = st.columns([3, 1.5, 1.5, 1.5, 2])
        sel_p = c_p.selectbox(f"제품 #{idx+1}", list(item_options.keys()), key=f"p_{idx}")
        p_obj = item_options[sel_p]
        
        unit_p = c_pr.number_input(t("unit_price"), value=float(p_obj["price"]), disabled=True, key=f"pr_{idx}")
        qty = c_q.number_input(t("qty"), min_value=0, max_value=p_obj["stock"], value=0, key=f"q_{idx}")
        
        box_count = round(qty / p_obj["items_per_box"], 2) if p_obj["items_per_box"] else qty
        c_bx.number_input(t("box_qty"), value=box_count, disabled=True, key=f"bx_{idx}")
        
        row_tot = unit_p * qty
        c_tot.number_input(t("total_amount"), value=row_tot, disabled=True, key=f"tot_{idx}")
