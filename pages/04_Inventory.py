...
# 1. 大吉商事 위탁 재고 (매입단가/원가 KRW 기준 총 평가액)
...
if daikichi:
    ...
    for row in dk_inv:
        p = row["products"]
        qty = row["stock_qty"]
        cost = float(p["purchase_cost_krw"])
        sum_krw = qty * cost
        tot_krw += sum_krw
        dk_rows.append({
            "SKU": p["SKU"],
...
# 2. 전체 SKU별 월간 재고 변동 (지난달 잔여/입고/출고/이번달 잔여)
