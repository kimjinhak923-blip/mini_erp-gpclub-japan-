...
with tab2:
    ...
    with st.form("add_product_form"):
        ...
        with col1:
            ...
            single_jan_code = st.text_input("낱장 JAN 코드 (마스크팩 등 필요시)", placeholder="선택 사항")
        ...
        if st.form_submit_button("상품 등록"):
            if not box_jan_code or not product_name:
                st.error("단상자 JAN 코드와 상품명은 필수입니다.")
            else:
...
with tab3:
    ...
    if st.session_state.master_fixtures:
        ...
        for fix in st.session_state.master_fixtures:
            ...
            # 출고 이력 중 해당 집기 출고량 자동 합산 및 차감
            out_q = sum(
                l.get("qty", 0) for l in logs 
                if l.get("product_name") == f_name and l.get("type") == "출고" and l.get("item_category") == "집기"
            )
